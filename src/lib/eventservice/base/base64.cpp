...
uint32 Base64::CalcDstSize(uint32 src_data_size) {
  return (((src_data_size + 2) / 3) * 4);
}

int Base64::EncodeFromArray(const unsigned char* data, size_t len,
                            char* result, int buf_size) {
  if ((NULL == data)
      || (0 == len)
      || (NULL == result)
      || (buf_size < ((len + 2) / 3) * 4)) {
...
void Base64::EncodeFromArray(MemBuffer::Ptr data, vzstd::string* result,
  ...
  vzes::BlocksPtr blks = data->blocks();
  vzes::BlocksPtr::iterator iter = blks.begin();
  for (size_t i = 0; i < nData && iter != blks.end(); iter++) {
    size_t cur_blk_size = (*iter)->buffer_size;
    const unsigned char* byte_data =
      static_cast<const unsigned char*>((*iter)->buffer);
    for (size_t j = 0; j < cur_blk_size; /*j++*/) {
      c = (byte_data[j] >> 2) & 0x3f;
      (*result)[dst_idx++] = Base64Table[c];

      c = (byte_data[j] << 4) & 0x3f;
      if (++j < nData) {
        c |= (byte_data[j] >> 4) & 0x0f;
      }
      (*result)[dst_idx++] = Base64Table[c];
...
void Base64::EncodeFromArray(MemBuffer::Ptr data, size_t start_block,
  ...
  uint32 count = 0;
  uint32 src_total_size = 0;
  size_t dest_ix = 0;
  BlocksPtr &blocks = data->blocks();
  for (BlocksPtr::iterator iter = blocks.begin();
    ...
    src_total_size += block->buffer_size;
    count++;

    unsigned char c;
    size_t i = 0;
    while (i < block->buffer_size) {
      c = (block->buffer[i] >> 2) & 0x3f;
      (*result)[dest_ix++] = Base64Table[c];

      c = (block->buffer[i] << 4) & 0x3f;
      if (++i < block->buffer_size) {
        c |= (block->buffer[i] >> 4) & 0x0f;
      }
      (*result)[dest_ix++] = Base64Table[c];
...
void Base64::EncodeFromArray(const void* data, size_t len,
  ...
  uint32 left_len = len;
  uint32 max_seg_len = DEFAULT_BLOCK_SIZE / 4 * 3;
  uint32 seg_num = (len % max_seg_len) ? (len / max_seg_len + 1) : (len / max_seg_len);
  ...
  for (uint32 i=0; i<seg_num; i++) {
    ...
    byte_data += i*max_seg_len;
    left_len  -= i*max_seg_len;
    curr_seg_len = (left_len > max_seg_len) ? (max_seg_len) : left_len;

    while (j < curr_seg_len) {
      c = (byte_data[j] >> 2) & 0x3f;
      block->buffer[dest_ix++] = Base64Table[c];
    ...
    block->buffer_size = ((curr_seg_len + 2) / 3) * 4;
...
void Base64::EncodeFromArray(const void* data, size_t len,
  ...
  const unsigned char* byte_data = static_cast<const unsigned char*>(data);
  // result->c_str() return a const pointer, we can casts it to char,
  // but not recommended.
  (void)EncodeFromArray(byte_data, len, (char*)(result->c_str()), buf_size);
}
